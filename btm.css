// ==UserScript==
// @name MooJaxNewHeal|| Dark Theme! | New HealBar! | Anti Ads! | BotMode! | Music! | ★Over 25.000 Installs★
// @version    2.8.5
// @description MooJax Menu![❚Macro = Z-Tank Gear + Shadow Wings | Space Bar - Blood Wings + Bull Helmet | C - Medic Gear + Angel Wings | M - Velocity | Esc - Modify❚]
// [❚Featured Hacker❚]
// [❚Musket + Katana = *,* to Katana and *.* to Musket❚]
// [❚Instakill Key = R❚] [❚Bow Insta Key = K ❚](The original!)[❚AutoHeal;Fast❚] [❚Little more hard to get Clown❚]
// [❚InstaKill Spike = On/Off❚]
// [❚KeyCodes = Y - BotMode | V - Spike | F - Pit | N - Windmill❚] [❚Circles = L - Spike | O - Pit | P - Windmill | J - Turret❚]
// [❚★Premium MAP★❚] [❚!AutoChat!❚] [❚!AutoSpawn!❚] !Discord! Server = https://discord.com/invite/GwzdWQy | !Discord! Dev.Name = Jack_LaL#8548
// @match        *://moomoo.io/*
...
// @Notices :1. MooJax last version will be 2.7.0 (Maybe will create experimental versions)
// @Changelog :1. Heal Bar Bug Fixes. 2. Force Connect, removed, there's a new one leaked 3. Bow Insta fixed 4. Instakill spike, fixed
// @Read : See my special of 1k video!
// @namespace -
// ==/UserScript==
...
document.getElementById("promoImgHolder").innerHTML = '</iframe><iframe width="300px"height="126.5px" src="https://www.youtube-nocookie.com/embed/qXQ86ILzSIA" frameborder="0" allowfullscreen></iframe>'; // Game music !! :D
...
$("#resDisplay").append(`<div id='healthDisplay' class='resourceDisplay'> <div align="right"><img src="https://vignette.wikia.nocookie.net/moom/images/8/81/Healing_Pad.png"></div></div>`);
...
setInterval(() => {
    if(messageToggle == 1) {
        prosender(["ch", [animate(true, 5)]])
    }
}, 200);

setInterval(() => {
    if(autoaim == true) {
        prosender(["2", [nearestEnemyAngle]]);
    }
}, 0);
...
var withtheme = true;
var rentheme = false;
//We having some issues with GreasyFork, but we'll update MooJax for you. Ever, Forever.
let healToggle = 1;
...
unsafeWindow.WebSocket.prototype.send = function(a){
    if (!ws){
        document.ws = this;
...
function socketFound(socket){
    socket.addEventListener('message', function(message){
        functioninterval(message);
    });
}
function functioninterval(a){
    let temp = msgpack5.decode(new Uint8Array(a.data));
    let data;
    if(temp.length > 1) {
        data = [temp[0], ...temp[1]];
    ...
    if (botmode){
    Walkbot = true;
    prosender(["33", [nearestEnemyAngle]]);
    prosender(["c", [1, null]]);
    autoaim = true;
    prosender(["13c", [0, 7, 0]]);
    } else {
...
function acc(id) {
    prosender(["13c", [0, 0, 1]]);
    prosender(["13c", [0, id, 1]]);
}

function hat(id) {
    prosender(["13c", [0, id, 0]]);
}
...
function placeF(id, angle = Math.atan2(mouseY - height / 2, mouseX - width / 2)) {
    ...
    prosender(["c", [1, angle]]);
    prosender(["c", [0, angle]]);
    prosender(["5", [myPlayer.weapon, true]]);
}
...
document.addEventListener('keydown', (e)=>{
    ...
    if (e.keyCode == 75 && document.activeElement.id.toLowerCase() !== 'chatbox') {//Bow insta, the original bro.
        prosender(["6", [9,]]);
        prosender(["5", [9, true]]);
        prosender(["c", [1]]);
        prosender(["6", [33]]);
    ...
    if (e.keyCode == 37 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        prosender(["13c", [0, HAT, 0]]);
        prosender(["13c", [0, ACC, 1]]);
    }
    ...
    if (e.keyCode == 39 && document.activeElement.id.toLowerCase() !== 'chatbox') {
        prosender(["13c", [0, HET, 0]]);
        prosender(["13c", [0, AEC, 1]]);
    }
...
autoaim = true;
        prosender(["5", [primary, true]]);
        prosender(["13c", [0, 7, 0]]);
        prosender(["13c", [0, 0, 1]]);
        prosender(["13c", [0, 18, 1]]);
        prosender(["c", [1]]);
        setTimeout( () => {
                    prosender(["13c", [0, 53, 0]]);
                    prosender(["13c", [0, 0, 1]]);
                    prosender(["13c", [0, 21, 1]]);
                    prosender(["5", [secondary, true]]);
        }, instaSpeed - 200);
        ...
        setTimeout( () => {
            ...
            prosender(["ch", [ichat(true, 5)]])
            } else {
             if (instapike == true) {
function placeSpike(id, angle = Math.atan2(mouseY - height / 2, mouseX - width / 2)) {
    prosender(["5", [id, null]]);
    prosender(["c", [1, angle]]);
    prosender(["c", [0, angle]]);
    prosender(["5", [myPlayer.weapon, true]]);
                 ...
                 let idd = spikeType
                 let anglezz = Math.atan2(mouseY - height / 2, mouseX - width / 2)
    prosender(["5", [idd, null]]);
    prosender(["c", [1, anglezz]]);
    prosender(["c", [0, anglezz]]);
    prosender(["5", [myPlayer.weapon, true]]);
                 autoaim = false;
             } else {
            prosender(["c", [1]]);
...
function update() {
    for (let i=0;i<9;i++){
        if (elementovisible(document.getElementById("actionBarItem" + i.toString()))){
            primary = i;
        }
    }
    ...
    for (let i=33;i<39;i++){
        if (elementovisible(document.getElementById("actionBarItem" + i.toString())) && i != 36){
            turretType = i - 16;
        }
    }
...
<div id="simpleModal" class="modal">
  ...
  <button id="butoon">Download Ice Mod!</button><button id="butoone">Download Moo Jax!</button>
        ...
        <div class="modal-footer">
            ...
            <p>Language: </p><div class="selectObj" id="lang1" style="display: block"><img id="wingprev" class="selPrev" src="https://esacademic.com/pictures/eswiki/70/Flag_of_the_United_States.svg" height="30" width="60"/></div>
            <div class="selectObj" id="lang2" style="display: none"><img id="wingprev" class="selPrev" src="data:image/png;base64," height="30" width="60"/></div>
            <div class="selectObj" id="lang3" style="display: none"><img id="wingprev" class="selPrev" src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/05/Flag_of_Brazil.svg/1200px-Flag_of_Brazil.svg.png" height="30" width="60"/></div>
            <p>Sub to (JackLaL)</p>
        </div>
    </div>
</div>
